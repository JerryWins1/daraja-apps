const CACHE='story-keeper-v11';
self.addEventListener('install',e=>{ e.waitUntil(caches.open(CACHE).then(c=>c.addAll(['./','./index.html'])).then(()=>self.skipWaiting())); });
self.addEventListener('activate',e=>{ e.waitUntil(caches.keys().then(ks=>Promise.all(ks.filter(k=>k.startsWith('story-keeper-')&&k!==CACHE).map(k=>caches.delete(k)))).then(()=>self.clients.claim())); });
